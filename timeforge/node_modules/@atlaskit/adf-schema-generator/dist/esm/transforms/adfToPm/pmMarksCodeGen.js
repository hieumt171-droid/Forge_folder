...
import { _codeBlock, _namedTypeImport, _interface, buildADFAttributesTypes, _functionCallToVariable, _type, convertTypeToTypeName, stringifyWithUndefined } from './codeGenHelpers';
...
export function pmMarksCodeGen(markResMap) {
  return _codeBlock(adfToPmAutogeneratedMessage, "import { createPMMarkSpecFactory, type PMMarkSpecFactoryInstance } from '../../schema/createPMSpecFactory'", _namedTypeImport('@atlaskit/editor-prosemirror/model', 'Mark'), objectToSortedArray(markResMap).map(function (_ref) {
    ...
    // build markSpec
    _functionCallToVariable(functionName, "createPMMarkSpecFactory".concat(markAttrs ? "<".concat(markName, ">") : ''), [stringifyWithUndefined(pmMarkSpec)], markAttrs ? "PMMarkSpecFactoryInstance<".concat(markName, ">") : 'PMMarkSpecFactoryInstance<Mark>'));
  }).join('\n\n'));
}