...
import { _codeBlock, _functionCallToVariable, _interface, _namedTypeImport, _type, buildADFAttributesTypes, convertTypeToTypeName, stringifyWithUndefined } from './codeGenHelpers';
...
export function pmNodesCodeGen(nodeResMap) {
  return _codeBlock(adfToPmAutogeneratedMessage, "import { createPMNodeSpecFactory, type PMNodeSpecFactoryInstance } from '../../schema/createPMSpecFactory'", _namedTypeImport('@atlaskit/editor-prosemirror/model', 'Node as PMNode'), _namedTypeImport.apply(void 0, ['./nodeGroupTypes'].concat(['InlineDefinition', 'BlockDefinition', 'BlockRootOnlyDefinition'])), _namedTypeImport.apply(void 0, ['./markTypes'].concat(_toConsumableArray(buildNodeMarksForImport(nodeResMap).sort()))), objectToSortedArray(nodeResMap).map(function (_ref2) {
    ...
    // build node speec
    _functionCallToVariable(functionName, "createPMNodeSpecFactory<".concat(nodeName, ">"), [stringifyWithUndefined(pmNodeSpec)], "PMNodeSpecFactoryInstance<".concat(nodeName, ">")));
  }).join('\n\n'));
}