...
var ADFNode = exports.ADFNode = /*#__PURE__*/function () {
  ...
  return (0, _createClass2.default)(ADFNode, [{
    ...
    key: "addContent",
    value: function addContent(child) {
      if (!_classPrivateFieldGet(_spec, this)) {
        throw new Error('Cannot addContent before define()');
      }

      // Case 1 — the node has no content yet. Seed it with `[$one+($or(child))]`.
      if (!_classPrivateFieldGet(_spec, this).content || _classPrivateFieldGet(_spec, this).content.length === 0) {
        var _content = [{
          type: '$one+',
          content: {
            type: '$or',
        ...
        _classPrivateFieldSet(_spec, this, _objectSpread(_objectSpread({}, _classPrivateFieldGet(_spec, this)), {}, {
          content: _content
        }));
        return this;
      }
      ...
      var existingLast = _classPrivateFieldGet(_spec, this).content[_classPrivateFieldGet(_spec, this).content.length - 1];
      var isQuantifier = (existingLast === null || existingLast === void 0 ? void 0 : existingLast.type) === '$one+' || (existingLast === null || existingLast === void 0 ? void 0 : existingLast.type) === '$zero+';
      var existingOrGroup = isQuantifier && !Array.isArray(existingLast.content) ? existingLast.content : undefined;
      var isOrGroup = (existingOrGroup === null || existingOrGroup === void 0 ? void 0 : existingOrGroup.type) === '$or' && Array.isArray(existingOrGroup.content);
      if (!isQuantifier || !isOrGroup || !existingOrGroup) {
        throw new Error("addContent expects existing content to end with a $one+($or(...)) or $zero+($or(...)) expression. Node type: ".concat(_classPrivateFieldGet(_type, this)));
      }

      // No-op if the child is already present — prevents duplicate schema entries.
      if (existingOrGroup.content.includes(child)) {
        return this;
      }
      ...
      var clonedOrGroup = _objectSpread(_objectSpread({}, existingOrGroup), {}, {
        content: [].concat((0, _toConsumableArray2.default)(existingOrGroup.content), [child])
      });
      var clonedQuantifier = _objectSpread(_objectSpread({}, existingLast), {}, {
        content: clonedOrGroup
      });
      var content = [].concat((0, _toConsumableArray2.default)(_classPrivateFieldGet(_spec, this).content.slice(0, -1)), [clonedQuantifier]);
      _classPrivateFieldSet(_spec, this, _objectSpread(_objectSpread({}, _classPrivateFieldGet(_spec, this)), {}, {
        content: content
      }));
      return this;
