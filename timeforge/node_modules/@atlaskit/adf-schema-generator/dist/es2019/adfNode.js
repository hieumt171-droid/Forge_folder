...
export class ADFNode {
  ...
  addContent(child) {
    ...
    if (!_classPrivateFieldGet(_spec, this).content || _classPrivateFieldGet(_spec, this).content.length === 0) {
      ...
      _classPrivateFieldSet(_spec, this, {
    ...
    const existingLast = _classPrivateFieldGet(_spec, this).content[_classPrivateFieldGet(_spec, this).content.length - 1];
    const isQuantifier = (existingLast === null || existingLast === void 0 ? void 0 : existingLast.type) === '$one+' || (existingLast === null || existingLast === void 0 ? void 0 : existingLast.type) === '$zero+';
    const existingOrGroup = isQuantifier && !Array.isArray(existingLast.content) ? existingLast.content : undefined;
    const isOrGroup = (existingOrGroup === null || existingOrGroup === void 0 ? void 0 : existingOrGroup.type) === '$or' && Array.isArray(existingOrGroup.content);
    if (!isQuantifier || !isOrGroup || !existingOrGroup) {
      throw new Error(`addContent expects existing content to end with a $one+($or(...)) or $zero+($or(...)) expression. Node type: ${_classPrivateFieldGet(_type, this)}`);
    }
    ...
    const clonedOrGroup = {
      ...existingOrGroup,
      content: [...existingOrGroup.content, child]
    };
    const clonedQuantifier = {
      ...existingLast,
      content: clonedOrGroup
    };
    const content = [..._classPrivateFieldGet(_spec, this).content.slice(0, -1), clonedQuantifier];
    _classPrivateFieldSet(_spec, this, {
      ..._classPrivateFieldGet(_spec, this),
      content
    });
