...
export function pmNodesCodeGen(nodeResMap) {
  return _codeBlock(adfToPmAutogeneratedMessage, `import { createPMNodeSpecFactory, type PMNodeSpecFactoryInstance } from '../../schema/createPMSpecFactory'`, _namedTypeImport('@atlaskit/editor-prosemirror/model', 'Node as PMNode'), _namedTypeImport('./nodeGroupTypes', ...['InlineDefinition', 'BlockDefinition', 'BlockRootOnlyDefinition']), _namedTypeImport('./markTypes', ...buildNodeMarksForImport(nodeResMap).sort()), objectToSortedArray(nodeResMap).map(([key, {
    nodeTypeDefinition,
    pmNodeSpec
  }]) => {
    ...
    // build node speec
    _functionCallToVariable(functionName, `createPMNodeSpecFactory<${nodeName}>`, [stringifyWithUndefined(pmNodeSpec)], `PMNodeSpecFactoryInstance<${nodeName}>`));
  }).join('\n\n'));
}