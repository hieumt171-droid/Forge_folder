...
export function pmMarksCodeGen(markResMap) {
  return _codeBlock(adfToPmAutogeneratedMessage, `import { createPMMarkSpecFactory, type PMMarkSpecFactoryInstance } from '../../schema/createPMSpecFactory'`, _namedTypeImport('@atlaskit/editor-prosemirror/model', 'Mark'), objectToSortedArray(markResMap).map(([key, {
    mark,
    pmMarkSpec
  }]) => {
    ...
    // build markSpec
    _functionCallToVariable(functionName, `createPMMarkSpecFactory${markAttrs ? `<${markName}>` : ''}`, [stringifyWithUndefined(pmMarkSpec)], markAttrs ? `PMMarkSpecFactoryInstance<${markName}>` : 'PMMarkSpecFactoryInstance<Mark>'));
  }).join('\n\n'));
}