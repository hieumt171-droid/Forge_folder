...
const ACCEPTED_SOURCES = ['forge-framework', 'forge-app', 'external', 'internal'];
...
export function shapeNavigationTimingData(data) {
  ...
  const timing = (payload === null || payload === void 0 ? void 0 : payload.timing) || payload;

  // Trim long URLs to just the last path segment to keep payload size manageable.
  const rawName = typeof payload.name === 'string' ? payload.name : '';
  const label = rawName.includes('/') ? (_rawName$split$filter = rawName.split('/').filter(Boolean).pop()) !== null && _rawName$split$filter !== void 0 ? _rawName$split$filter : rawName : rawName;
  return {
    label,
...
export function shapeResourceTimingData(data) {
  ...
  const timing = (payload === null || payload === void 0 ? void 0 : payload.timing) || payload;
  const rawName = typeof payload.name === 'string' ? payload.name : '';
  const name = rawName.includes('/') ? (_rawName$split$filter2 = rawName.split('/').filter(Boolean).pop()) !== null && _rawName$split$filter2 !== void 0 ? _rawName$split$filter2 : rawName : rawName;
  ...
  const encodedSize = typeof payload.encodedBodySize === 'number' ? payload.encodedBodySize : undefined;
  const decodedSize = typeof payload.decodedBodySize === 'number' ? payload.decodedBodySize : undefined;
  ...
  const source = ACCEPTED_SOURCES.includes(payload.source) ? payload.source : undefined;
