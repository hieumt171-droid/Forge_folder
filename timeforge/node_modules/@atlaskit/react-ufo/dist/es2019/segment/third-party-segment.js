...
import { addCompletedHold, addIframeSegmentData, addSegmentExtraData, getActiveInteraction } from '../interaction-metrics';
import UFOLoadHold from '../load-hold/UFOLoadHold';
import { shapeNavigationTimingData, shapeResourceTimingData } from '../resource-timing/common/utils/shape-resource-timing';
import UFOSegment from './segment';
import { isDomMutationsFinalBatch, shapeDomMutationsData, shapeLargestContentfulPaintData, shapeLayoutShiftData, shapePaintTimingData } from './shape-iframe-dom-events';
...
const IFRAME_SEGMENT_DATA_SUFFIXES = ['resource-timing', 'paint-timing', 'largest-contentful-paint', 'layout-shift', 'dom-mutations', 'navigation-timing'];
...
function IframeSegmentIdReader({
  ...
  const labelStack = context === null || context === void 0 ? void 0 : context.labelStack;
  const segmentId = labelStack && labelStack.length > 0 ? labelStack[labelStack.length - 1].segmentId : undefined;
...
function IframeSegment({
  ...
  const segmentIdRef = useRef(undefined);
  ...
  const resizedCounterSegmentIdRef = useRef(undefined);
  const isUiKit = appType === 'UIKit';

  // Released on the first navigation-timing event from the iframe.
  const [navigationHold, setNavigationHold] = useState('holding');
  ...
  const profilerAccRef = useRef(null);
  useLayoutEffect(() => {
    ...
    interactionContext.onRender = function (phase, actualDuration, baseDuration, startTime, commitTime) {
  ...
  useEffect(() => {
    ...
    const finaliseHolds = () => {
      ...
      const lStack = interactionContext === null || interactionContext === void 0 ? void 0 : interactionContext.labelStack;
      if (iid && lStack) {
    ...
    const handleEvent = event => {
      ...
      if (event.type === 'resized') {
        ...
        if (typeof height === 'number' && Number.isFinite(height) && height >= 0 && typeof elapsed === 'number' && Number.isFinite(elapsed) && elapsed >= 0 && resizedCountRef.current < MAX_RESIZED_ENTRIES_PER_SEGMENT && interactionId.current && segmentIdRef.current) {
      ...
      if (event.type !== 'ufo-event' || typeof event.name !== 'string' || !event.name.startsWith(FORGE_TTAI_EVENT_PREFIX)) {
        return;
      }
      ...
      if (eventName === FORGE_INIT_EVENT && !iframeEventsReceived) {
        ...
        clearTimeout(abortTimer);
        const remainingMs = Math.max(ABORT_TIMEOUT_EXTENDED_MS - (performance.now() - mountTime), 0);
        abortTimer = setTimeout(() => {
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(UFOLoadHold, {
    name: "forge-ui-requests",
    hold: navigationHold === 'holding'
  }), /*#__PURE__*/React.createElement(IframeSegmentIdReader, {
...
function SegmentExtraDataWriter({
  ...
  const interactionContext = useContext(UFOInteractionContext);
  const interactionId = useContext(UFOInteractionIDContext);
  const labelStack = interactionContext === null || interactionContext === void 0 ? void 0 : interactionContext.labelStack;
  const segmentId = labelStack && labelStack.length > 0 ? labelStack[labelStack.length - 1].segmentId : undefined;
  useEffect(() => {
...
export const UFOThirdPartySegment = props => {
  ...
  return /*#__PURE__*/React.createElement(UFOSegment, _extends({
    type: "third-party"
  }, otherProps), onRegisterIframeEventListener && extraData && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(IframeSegment, {
    onRegisterIframeEventListener: onRegisterIframeEventListener,
    appType: extraData.appType
  }), /*#__PURE__*/React.createElement(SegmentExtraDataWriter, {
    extraData: extraData
  })), children);
};
