...
export const SsrRenderProfilerInner = ({
  ...
  const reactProfilerId = useMemo(() => labelStack.map(l => sanitizeLabelStackName(l.name)).join('/'), [labelStack]);
  checkActiveInteractionAndResetStartMarksIfSet();
  const parentSpan = useContext(ParentSpanContext);
  const currentSpanState = onStartRender(reactProfilerId, sanitizeLabelStackName(labelStack[labelStack.length - 1].name), parentSpan);
