...
const CHUNK_LOAD_ERROR_MESSAGES = ['Unable to fetch manifest', 'dynamically imported module', 'Importing a module script failed'];
const NETWORK_CLIENT_ERROR_NAMES = new Set(['NetworkError', 'Network error']);
const NETWORK_CLIENT_ERROR_MESSAGES = ['Failed to fetch', 'Load failed', 'Network request failed', 'NetworkError', 'server with the specified hostname could not be found', 'The network connection was lost', 'The request timed out', 'connection failure', 'CloseEvent: Ping timeout', 'Curl Error: Failure when receiving data from the peer'];
const NETWORK_SERVER_ERROR_MESSAGES = ['... is not valid JSON', 'in JSON at position 0', 'JSON.parse: unexpected character', 'JSON.parse: unexpected end of data', 'Unexpected end of input', 'Unexpected end of JSON input', 'cannot parse response', 'Syntax or http error'];
function matchesCategory(error) {
  var _error$name, _error$message;
  const name = (_error$name = error.name) !== null && _error$name !== void 0 ? _error$name : '';
  const message = (_error$message = error.message) !== null && _error$message !== void 0 ? _error$message : '';
  if (ABORT_ERROR_NAMES.has(name) || ABORT_ERROR_MESSAGES.some(pattern => message.includes(pattern)) ||
  // OAuth Abort errors
  AUTH_ERROR_NAMES.has(name) && AUTH_ERROR_ABORT_MESSAGES.some(pattern => message.includes(pattern))) {
    return 'abort';
  }
  if (AUTH_ERROR_NAMES.has(name)) {
    return 'auth';
  }
  if (CHUNK_LOAD_ERROR_NAMES.has(name) || CHUNK_LOAD_ERROR_MESSAGES.some(pattern => message.includes(pattern))) {
    return 'chunk-load';
  }
  if (NETWORK_CLIENT_ERROR_NAMES.has(name) || NETWORK_CLIENT_ERROR_MESSAGES.some(pattern => message.includes(pattern))) {
...
export function classifyTerminalError(error) {
  ...
  if (typeof error === 'object' && error !== null && 'cause' in error && error.cause instanceof Error) {
