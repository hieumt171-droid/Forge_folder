...
const encodeRect = rect => [Math.round(rect.left * 10) / 10, Math.round(rect.top * 10) / 10, Math.round(rect.right * 10) / 10, Math.round(rect.bottom * 10) / 10];
const areRectsEqual = (left, right) => left.every((value, index) => value === right[index]);
...
export default class RawDataHandler {
  ...
  async getRawData({
    ...
    let rawObservations = viewportEntries.map(entry => {
      ...
      const encodedPreviousRect = viewportEntry.previousRect ? encodeRect(viewportEntry.previousRect) : null;
      const shouldIncludePreviousRect = encodedPreviousRect === null || !areRectsEqual(encodedRect, encodedPreviousRect);
      const observation = {
    ...
    if (shouldTrimViewportObservations || shouldTrimEventObservations) {
      ...
      const referencedEventTargetEids = new Set();
    ...
    const scrubbedObservations = isFedrampScrubbed ? rawObservations.map(({
      att: _att,
      ...rest
    }) => rest) : rawObservations;
    const result = {
      revision: this.revisionNo,
      clean: isVCClean,
      'metric:vc90': null,
