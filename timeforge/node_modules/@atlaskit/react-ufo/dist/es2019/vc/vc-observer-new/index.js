...
export default class VCObserverNew {
  constructor(config) {
    var _config$isPostInterac, _ref, _getSelectorConfig, _config$trackLayoutSh;
    ...
    this.windowEventObserver = new WindowEventObserver({
      onEvent: ({
        ...
        const abortEventTargetElement = isAbortEventTargetSupported(type) && event ? getAbortEventTargetElement(event.target) : null;
        const elementName = abortEventTargetElement ? this.getElementName(abortEventTargetElement) : undefined;
        this.entriesTimeline.push({
          time,
          data: {
            type: 'window:event',
  ...
  async getVCResult(param) {
    ...
    const [fy26_04, vcNext] = await Promise.all([isFy2604Enabled ? calculator_fy26_04.calculate(calculatorParams) : null, isVCRevisionEnabled('next') ? calculator_next.calculate(calculatorParams) : null]);
    if (fy26_04) {
    ...
    const shouldIncludeRawData = includeRawData || !isFy2604Enabled || fg('platform_ufo_always_emit_raw_handler');
...
function labelStackToSegment(labelStack) {
  ...
  return labelStack.slice(0, segmentIndex + 1).map(label => sanitizeLabelStackName(label.name)).join('/');
}
