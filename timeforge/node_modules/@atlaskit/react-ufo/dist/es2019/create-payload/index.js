...
function getResourceTimingsPayload(resourceTimings) {
  const legacyResourceTimings = objectToArray(resourceTimings);
...
function getReactProfilerTimingsByMetricWindow(interaction, reactUFOVersion, registry) {
  ...
  const entries = Object.entries(interaction.metricWindows).filter(([name]) => name !== 'standard').map(([name, window]) => [name, optimizeReactProfilerTimings(getReactProfilerTimingsForWindow(interaction.reactProfilerTimings, window), interaction.start, reactUFOVersion, registry)]);
  if (entries.length === 0) {
    return {};
  }
  return {
...
async function createInteractionMetricsPayload(interaction, interactionId, criticalPayloadCount, vcMetrics) {
  ...
  const registry = reactUFOVersion === '2.0.0' && fg('platform_ufo_labelstack_dedup') ? new LabelStackRegistry() : undefined;
  const segments = !fg('platform_ufo_remove_deprecated_config_fields') && config.killswitchNestedSegments ? [] : knownSegments;
  const segmentTree = reactUFOVersion === '2.0.0' ? buildSegmentTree(segments.map(segment => segment.labelStack)) : {};
  ...
  const calculatePageVisibilityFromTheStartOfPageLoad = ((_config$enableBetterP = config.enableBetterPageVisibilityApi) !== null && _config$enableBetterP !== void 0 ? _config$enableBetterP : fg('platform_enable_better_page_visibility')) && isPageLoadEvent;
  ...
  const getDetailedInteractionMetrics = resourceTimings => {
    ...
    const metricVariantPayload = interaction.metricWindows ? fg('platform_ufo_metric_variants') ? {
      ...getReactProfilerTimingsByMetricWindow(interaction, reactUFOVersion, registry),
      metricWindows: Object.fromEntries(Object.entries(interaction.metricWindows).map(([name, window]) => [name, window ? {
        ...window,
    ...
    const basePayload = {
      ...
      ...(interaction.segment3pTimings && interaction.segmentExtraData ? fg('platform_ufo_ecosystem_data_in_payload') ? (() => {
        const grouped = buildSegment3pData(interaction.segment3pTimings, interaction.segmentExtraData);
        if (!grouped) {
          return {};
        }
  ...
  const getPageLoadDetailedInteractionMetrics = () => {
    ...
    const ssrTimingsFromConfig = config === null || config === void 0 ? void 0 : (_config$ssr2 = config.ssr) === null || _config$ssr2 === void 0 ? void 0 : (_config$ssr2$getSSRTi = _config$ssr2.getSSRTimings) === null || _config$ssr2$getSSRTi === void 0 ? void 0 : _config$ssr2$getSSRTi.call(_config$ssr2);
    return {
      initialPageLoadExtraTimings,
      SSRTimings: ssrTimingsFromConfig ? [...ssrTimingsFromConfig, ...defaultSSRTimings] : defaultSSRTimings
    };
  };
  const newUFOName = sanitizeUfoName(ufoName);
  const resourceTimings = getResourceTimings(start, end);
  const standardReactProfilerTimings = fg('platform_ufo_metric_variants') ? getReactProfilerTimingsForWindow(interaction.reactProfilerTimings, (_interaction$metricWi = interaction.metricWindows) === null || _interaction$metricWi === void 0 ? void 0 : _interaction$metricWi.standard) : interaction.reactProfilerTimings;
  const [finalVCMetrics, paintMetrics, batteryInfo] = await Promise.all([vcMetrics || (await getVCMetrics(interaction)), getPaintMetricsToLegacyFormat(type, end), getBatteryInfoToLegacyFormat()]);
  ...
  const interactionMetricsFieldsToTrim = fg('ufo_remove_featureflags_from_trimmed_fields') ? ['requestInfo', 'resourceTimings', 'segment3pData', 'segment3pTimingAborts'] : ['requestInfo', 'featureFlags', 'resourceTimings', 'segment3pData', 'segment3pTimingAborts'];
...
export async function createPayloads(interactionId, interaction) {
  ...
  const interactionMetricsPayload = await createInteractionMetricsPayload(modifiedInteraction, interactionId, criticalPayloadCount, vcMetrics);
...
export async function createExtraSearchPageInteractionPayload(interactionId, interaction) {
  ...
  const interactionMetricsPayload = await createInteractionMetricsPayload(modifiedInteraction, newInteractionId, undefined, vcMetrics);
  payloads.push(interactionMetricsPayload);
  return payloads.filter(Boolean);
}