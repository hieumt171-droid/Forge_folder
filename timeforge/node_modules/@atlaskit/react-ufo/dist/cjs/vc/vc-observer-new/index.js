...
var VCObserverNew = exports.default = /*#__PURE__*/function () {
  function VCObserverNew(config) {
    ...
    this.selectorConfig = (_ref = (_getSelectorConfig = (0, _config.getSelectorConfig)(config.selectorConfig, DEFAULT_SELECTOR_CONFIG)) !== null && _getSelectorConfig !== void 0 ? _getSelectorConfig : config.selectorConfig) !== null && _ref !== void 0 ? _ref : DEFAULT_SELECTOR_CONFIG;
    ...
    this.windowEventObserver = new _windowEventObserver.default({
      onEvent: function onEvent(_ref2) {
        ...
        var abortEventTargetElement = isAbortEventTargetSupported(type) && event ? getAbortEventTargetElement(event.target) : null;
        var elementName = abortEventTargetElement ? _this.getElementName(abortEventTargetElement) : undefined;
        _this.entriesTimeline.push({
          time: time,
          data: _objectSpread({
            type: 'window:event',
  ...
  return (0, _createClass2.default)(VCObserverNew, [{
      ...
      if ((0, _platformFeatureFlags.fg)('ufo_fix_aborting_interaction_detection_during_ssr')) {
        ...
        if ((_window2 = window) !== null && _window2 !== void 0 && _window2.__SSR_ABORT_LISTENERS__) {
          ...
          if (_aborts && (0, _typeof2.default)(_aborts) === 'object') {
            Object.entries(_aborts).forEach(function (_ref5) {
    ...
    value: function () {
      var _getVCResult = (0, _asyncToGenerator2.default)( /*#__PURE__*/_regenerator.default.mark(function _callee(param) {
        var start, stop, interactionId, interactionType, interactionAbortReason, isPageVisible, include3p, includeSSRRatio, excludeSmartAnswersInSearch, includeRawData, rawDataStopTime, results, feVCCalculationStartTime, calculator_fy25_03, orderedEntries, fy25_03, calculator_fy26_04, calculator_next, calculatorParams, isFy2604Enabled, _yield$Promise$all, _yield$Promise$all2, fy26_04, vcNext, feVCCalculationEndTime, shouldIncludeRawData, rawVCCalculationStartTime, rawHandler, rawStopTime, rawOrderedEntries, raw, _t;
        return _regenerator.default.wrap(function (_context) {
          while (1) switch (_context.prev = _context.next) {
            ...
            case 3:
              ...
              isFy2604Enabled = (0, _config.isVCRevisionEnabled)('fy26.04');
              _context.next = 4;
              return Promise.all([isFy2604Enabled ? calculator_fy26_04.calculate(calculatorParams) : null, (0, _config.isVCRevisionEnabled)('next') ? calculator_next.calculate(calculatorParams) : null]);
            case 4:
              ...
              feVCCalculationEndTime = performance.now(); // Always include raw data when fy26.04 is not in the client config so the server
              // can recalculate fy26.04 metrics (ssrRatio, labelStacks, speedIndex) from raw observations.
              shouldIncludeRawData = includeRawData || !isFy2604Enabled || (0, _platformFeatureFlags.fg)('platform_ufo_always_emit_raw_handler');
...
function labelStackToString(labelStack) {
  return labelStack.map(function (label) {
    return (0, _utils.sanitizeLabelStackName)(label.name);
  }).join('/');
}
...
var SUPPORTED_ABORT_EVENT_TARGET_TYPES = new Set(['scroll-container', 'scroll', 'wheel']);
function isAbortEventTargetSupported(type) {
  return SUPPORTED_ABORT_EVENT_TARGET_TYPES.has(type);
}
function getAbortEventTargetElement(target) {
  if (target instanceof HTMLElement) {
    return target;
  }
  if (target instanceof Document) {
    var _target$documentEleme;
    var scrollingElement = target.scrollingElement;
    if (scrollingElement instanceof HTMLElement) {
      return scrollingElement;
    }
    return (_target$documentEleme = target.documentElement) !== null && _target$documentEleme !== void 0 ? _target$documentEleme : null;
  }
  if (target instanceof Window) {
    var _target$document$docu;
    var _scrollingElement = target.document.scrollingElement;
    if (_scrollingElement instanceof HTMLElement) {
      return _scrollingElement;
    }
    return (_target$document$docu = target.document.documentElement) !== null && _target$document$docu !== void 0 ? _target$document$docu : null;
  }
