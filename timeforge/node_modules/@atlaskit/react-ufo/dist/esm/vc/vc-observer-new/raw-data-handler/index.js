...
var encodeRect = function encodeRect(rect) {
  return [Math.round(rect.left * 10) / 10, Math.round(rect.top * 10) / 10, Math.round(rect.right * 10) / 10, Math.round(rect.bottom * 10) / 10];
};
var areRectsEqual = function areRectsEqual(left, right) {
  ...
  return _createClass(RawDataHandler, [{
    ...
    value: function () {
      var _getRawData = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime.mark(function _callee(_ref) {
        var entries, startTime, stopTime, isPageVisible, isVCClean, dirtyReason, getVCCleanStatusResult, _getEarliestHiddenTim, viewportEntries, windowEventEntries, targetNameToIdMap, elementMapEntriesMap, nextElementId, typeMap, typeMapEntriesMap, nextTypeId, attributeMap, attributeEntriesMap, nextAttributeId, eventTypeMap, eventTypeMapEntriesMap, nextEventTypeId, labelStacksMap, rawObservations, rawEventObservations, shouldTrimEventObservations, shouldTrimViewportObservations, firstEventObservation, lastEventObservations, firstObservation, lastObservations, ssrEid, ssrObservation, ssrAlreadyIncluded, referencedEids, referencedChgs, referencedAtts, referencedEvts, referencedEventTargetEids, _iterator, _step, observation, _iterator2, _step2, _observation, _iterator3, _step3, eid, _iterator4, _step4, chg, _iterator5, _step5, att, _iterator6, _step6, evt, isFedrampScrubbed, scrubbedObservations, result;
          ...
          while (1) switch (_context.prev = _context.next) {
            case 0:
              ...
              return _context.abrupt("return", {
                ...
                abortTimestamp: (_getEarliestHiddenTim = getEarliestHiddenTiming(startTime, stopTime)) !== null && _getEarliestHiddenTim !== void 0 ? _getEarliestHiddenTim : -1,
                viewport: {
                  w: getViewportWidth(),
                  h: getViewportHeight()
                }
            ...
            case 1:
              ...
              rawObservations = viewportEntries.map(function (entry) {
                ...
                if (viewportEntry.labelStacks && !(eid in labelStacksMap)) {
                  ...
                  var shouldCompactImplicitUnknown = labelInfo.s === 'unknown' && labelInfo.l === 'unknown';
                  labelStacksMap[eid] = shouldCompactImplicitUnknown ? 'u' : labelInfo;
                }
                ...
                var encodedRect = encodeRect(rect);
                var encodedPreviousRect = viewportEntry.previousRect ? encodeRect(viewportEntry.previousRect) : null;
                var shouldIncludePreviousRect = encodedPreviousRect === null || !areRectsEqual(encodedRect, encodedPreviousRect);
                var observation = _objectSpread(_objectSpread({
                  t: Math.round(entry.time - startTime),
                  r: encodedRect
                }, shouldIncludePreviousRect ? {
                  pr: encodedPreviousRect
                } : {}), {}, {
                  chg: chg,
                  eid: eid || 0
                }, att > 0 ? {
              ...
              rawEventObservations = windowEventEntries.map(function (entry) {
                ...
                var eventTargetId = 0;
                var eventTargetName = windowEventEntry.elementName || '';
                if (eventTargetName) {
                ...
                var eventObservation = _objectSpread({
                  t: Math.round(entry.time - startTime),
                  evt: evtId
                }, eventTargetId > 0 ? {
                  eid: eventTargetId
              ...
              shouldTrimEventObservations = rawEventObservations.length > MAX_OBSERVATIONS;
              shouldTrimViewportObservations = rawObservations.length > MAX_OBSERVATIONS; // If the number of event observations is greater than the maximum allowed, we need to trim the event observations to the maximum allowed.
              ...
              if (shouldTrimViewportObservations || shouldTrimEventObservations) {
                ...
                referencedEventTargetEids = new Set();
                ...
                _iterator2 = _createForOfIteratorHelper(rawEventObservations);
                try {
                  for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
                    _observation = _step2.value;
                    if (_observation.evt > 0) {
                      referencedEvts.add(_observation.evt);
                    }
                    if (_observation.eid !== undefined && _observation.eid > 0) {
                      referencedEventTargetEids.add(_observation.eid);
                    }
                  }
                } catch (err) {
                  _iterator2.e(err);
                ...
                _iterator3 = _createForOfIteratorHelper(Object.keys(elementMapEntriesMap).map(Number));
                try {
                ...
                if (shouldTrimViewportObservations) {
                  _iterator4 = _createForOfIteratorHelper(Object.keys(typeMapEntriesMap).map(Number));
                  try {
                  ...
                  _iterator5 = _createForOfIteratorHelper(Object.keys(attributeEntriesMap).map(Number));
                  try {
              ...
              isFedrampScrubbed = isFedrampOverrideActive();
              scrubbedObservations = isFedrampScrubbed ? rawObservations.map(function (_ref2) {
                var _att = _ref2.att,
              ...
              result = {
                ...
                'metric:vc90': null,
                rawData: {
                  obs: scrubbedObservations !== null && scrubbedObservations !== void 0 ? scrubbedObservations : undefined,
                  ...
                  att: isFedrampScrubbed ? undefined : attributeEntriesMap !== null && attributeEntriesMap !== void 0 ? attributeEntriesMap : undefined,
                  ...
                  lbl: isFedrampScrubbed ? undefined : Object.keys(labelStacksMap).length > 0 ? labelStacksMap : undefined,
                  lblMode: isFedrampScrubbed ? undefined : Object.keys(labelStacksMap).length > 0 ? 'sentinel-v1' : undefined
                },
