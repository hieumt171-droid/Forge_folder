...
import { isSegmentLabel, sanitizeLabelStackName, sanitizeUfoName, stringifyLabelStackFully } from '../create-payload/common/utils';
  ...
  var reactProfilerTimingsMap = filtered.reduce(function (result, _ref) {
    ...
    if (labelStack && type !== 'nested-update') {
      var label = labelStack.map(function (ls) {
        return sanitizeLabelStackName(ls.name);
      }).join('/');
