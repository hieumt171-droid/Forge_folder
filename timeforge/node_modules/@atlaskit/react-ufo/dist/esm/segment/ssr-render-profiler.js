...
export var SsrRenderProfilerInner = function SsrRenderProfilerInner(_ref2) {
  ...
  var reactProfilerId = useMemo(function () {
    return labelStack.map(function (l) {
      return sanitizeLabelStackName(l.name);
    }).join('/');
  }, [labelStack]);
  checkActiveInteractionAndResetStartMarksIfSet();
  var parentSpan = useContext(ParentSpanContext);
  var currentSpanState = onStartRender(reactProfilerId, sanitizeLabelStackName(labelStack[labelStack.length - 1].name), parentSpan);
